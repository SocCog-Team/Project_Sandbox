function [] = rn_PlotAvgCombined_Vergence_different_conditions_blocked_BIFR (FullStructure, FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1:8
	...
	if (a==1) || (a==3) || (a==5) || (a==7)
		cur_fieldnames_blocked_A_newcondition = fieldnames_blocked_A{a+1};
	else
		continue
	end
	TrialLists_blocked_A = FullStructure_blocked.A.(cur_fieldnames_blocked_A);
	tmp_TrialLists_blocked_A = FullStructure_blocked.A.(cur_fieldnames_blocked_A_newcondition);
	...
	%     SpBIFRAlignedBIFR=BIFRsEpoch.(Sp{1});
	%
	...
	A_blocked_isNotNaN_array = ~isnan(TrialLists_blocked_A);
	...
	tmp_AvgA_blocked = mean(tmp_TrialLists_blocked_A, 1, 'omitnan');
	tmp_STD_A_blocked = std(tmp_TrialLists_blocked_A, 'omitnan');
	tmp_SEM_A_blocked = std(tmp_TrialLists_blocked_A, 'omitnan') / size(tmp_TrialLists_blocked_A, 1);
	tmp_A_blocked_isNotNaN_array = ~isnan(tmp_TrialLists_blocked_A);
	tmp_A_blocked_N_per_bin_list = sum(tmp_A_blocked_isNotNaN_array, 1);
	[tmp_A_blocked_CIHW95] = calc_cihw(tmp_STD_A_blocked,tmp_A_blocked_N_per_bin_list, 0.05);
	%   [A_CIHW95] = calc_cihw(STD_A, (size((TrialListsAA),1)), 0.05);
	tmp_A_blocked_CIHW95(tmp_A_blocked_CIHW95 == 0) = NaN;
...
% 	end
	...
	hold on
	[AClosertoZero BClosertoZero] = tn_ClosertoZero(AvgA_blocked,tmp_AvgA_blocked, TrialListsTimepoints);
	%      hold on
	...
	hold on
	plot((TrialListsTimepoints(1,:)),AvgA_blocked,'Color','r', 'LineWidth',2);
	hold on
	plot((TrialListsTimepoints(1,:)),(AvgA_blocked+SEM_A_blocked),'Color','r', 'LineWidth',0.5);
	plot((TrialListsTimepoints(1,:)),AvgA_blocked-SEM_A_blocked,'Color','r', 'LineWidth',0.5);
	hold on
	plot((TrialListsTimepoints(1,:)),tmp_AvgA_blocked,'Color','b', 'LineWidth',2);
	plot((TrialListsTimepoints(1,:)),tmp_AvgA_blocked + tmp_SEM_A_blocked,'Color','b', 'LineWidth',0.5);
	plot((TrialListsTimepoints(1,:)),tmp_AvgA_blocked - tmp_SEM_A_blocked,'Color','b', 'LineWidth',0.5);
	hold on
	...
	%     plot((TrialListsTimepoints(6,:)),AvgAT_blocked - SEM_AT_blocked,'Color','m', 'LineWidth',0.5);
	%     hold on
	plot((TrialListsTimepoints(1,:)),AvgA_blocked + A_blocked_CIHW95,'Color','r', 'LineWidth',0.5, 'LineStyle', '--');
	plot((TrialListsTimepoints(1,:)),AvgA_blocked - A_blocked_CIHW95,'Color','r', 'LineWidth',0.5, 'LineStyle', '--');
	plot((TrialListsTimepoints(1,:)),tmp_AvgA_blocked + tmp_A_blocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	plot((TrialListsTimepoints(1,:)),tmp_AvgA_blocked - tmp_A_blocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	...
	if b==1
		ylabel('Avg Vergence blocked and Vergence blocked BIFR','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoBIFR.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoBIFR.pdf'))
	end
	...
	%     if a>=9 && a<=12
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveRightBottomAlignedtoBIFR.jpg']))
	%
	%     end
	%     if a>=13 && a<=16
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftTopAlignedtoBIFR.jpg']))
	%
	%     end
	%     if a>=17 && a<=20
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftCenterAlignedtoBIFR.jpg']))
	%
	%     end
	%     if a>=21 && a<=24
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftBottomAlignedtoBIFR.jpg']))
	%     end
...
for a=1:length(Clusters);
	...
	PlotClusterTimes(PlotClusterTimes==0)=45;
...
if length(ListSignClusters(~isnan(ListSignClusters)))>0
	for a=1:length(ListSignClusters)
		...
		PlotSignClusterTimes(PlotSignClusterTimes==0)=50;
...
ClosertoZeroY(ClosertoZeroY==0) = -50;
