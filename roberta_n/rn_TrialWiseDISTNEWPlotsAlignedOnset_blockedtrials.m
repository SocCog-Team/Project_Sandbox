...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A]= rn_TrialWiseDISTNEWPlotsAlignedOnset_blockedtrials(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
...
% 		DistTouchB_finaltarget_B.Type2408 = vertcat(DistTouchB_finaltarget_B.Type2408,distTouchB_finaltarget_B(TrialLists,:));
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_Red_ObjectiveRightTopAlignedtoOnset.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_Red_ObjectiveRightCenterAlignedtoOnset.pdf'))
		
	end
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_Red_ObjectiveRightBottomAlignedtoOnset.pdf'))
		
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_Red_ObjectiveLeftTopAlignedtoOnset.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_Red_ObjectiveLeftCenterAlignedtoOnset.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_Red_ObjectiveLeftBottomAlignedtoOnset.pdf'))
	end
end
