function [] = rn_PlotAvgCombined_Touch_different_conditions_blocked_AIFR (FullStructure, FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if b==1
		ylabel('Avg Touch A blocked and Touch A blocked AIFR','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_blockedVSTouchAblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoAIFR.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_blockedVSTouchAblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoAIFR.pdf'))
	end
