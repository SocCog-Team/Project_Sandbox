function [] = rn_PlotAvgCombined_Vergence_different_conditions_unblocked_BIFR (FullStructure, FullStructure_unblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if b==1
		ylabel('Avg Vergence unblocked and Vergence unblocked BIFR','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_unblockedVSVergenceunblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoBIFR.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_unblockedVSVergenceunblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoBIFR.pdf'))
	end
