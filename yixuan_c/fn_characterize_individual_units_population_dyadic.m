function [] = fn_characterize_individual_units_population_dyadic(raster_dir_relative_to_alignment, initial_alignment_event, PETHdata_base_dir, proto_unit_list, n_units, alignment_event_list, output_directory, session_ID)
...
if dyadic_analysis ~= 1
	for i_alignment = 1 : length(alignment_event_list)
		...
		for i_factor = 1: 2 % 2 as length(factor_unique_instances)
			...
			sig_idx = zeros([3, n_units]);
			insig_idx = zeros([3, n_units]);
...
% 				goodtrial_idx = intersect(goodtrial_idx, cur_factor_idx);
				...
				[p, tbl, stats, terms] = anovan(firing_rate_Hz(goodtrial_idx), {factor_list(goodtrial_idx),B_factor_list(goodtrial_idx)},'model',2,'varnames',{'TrialType','DiffGo'}, 'display','off');
				...
				for i_p = 1:length(p)
					if p(i_p)<0.05
						sig_idx(i_p,i_unit) = i_unit;
					else
						% 					if i_unit == 1
						% 						i_coloumn = 1;
						% 					end
						insig_idx(i_p, i_unit)= i_unit;
					end
				end
			...
			for i_plot = 1:length(p)
				...
				ylabel('Counts');
% 				sgtitle({['Alignment: ', cur_alignment, ' Two-way anova of TrialType%DiffGo in B<A']}, 'Interpreter', 'None');
				sgtitle({['Alignment: ', cur_alignment, ' Two-way anova of A_LR & B_LR.',set.value]}, 'Interpreter', 'None');
			...
			%
			col = ["A_LR","B_LR","A_LR&B_LR","Counts","Percentage"];
			row = {'No significant','A_LR only','B_LR only','interaction only','A_LR andn B_LR','A_LR and interaction','B_LR and interaction','All significant','Counts','Percentage'};
			...
			sig_count_list(end+1)= n_units;
			sig_count_list(1)= n_units - length(nonzeros(sig_idx(1,:)));
...
% 			writetable(sig_table,fullfile(output_directory,['Percentage of significant units of TrialType&DiffGo', '.', factor_uniqe_instances{1+i_factor}, '.', cur_alignment, '.', '.csv']),"WriteRowNames",true);
					writetable(sig_table,fullfile(output_directory,['Percentage of significant units of sel and partner actions.A_faster_B','.', cur_alignment, '.csv']),"WriteRowNames",true);
			
% 			write_out_figure(fh_population, fullfile(output_directory, ['Summary of two way-ANOVA LR&DiffGo', '.',   factor_uniqe_instances{1+i_factor}, '.', cur_alignment, '.', '.pdf']));
					write_out_figure(fh_population, fullfile(output_directory, ['Summary of two way-ANOVA of sel and partner actions.A_faster_B','.', cur_alignment, '.', '.pdf']));
