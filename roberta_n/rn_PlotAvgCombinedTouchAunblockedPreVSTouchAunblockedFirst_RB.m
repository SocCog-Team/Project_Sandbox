function [] = rn_PlotAvgCombinedTouchAunblockedPreVSTouchAunblockedFirst_RB (FullStructure, FullStructure_unblocked,FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1:8
	...
	if b==1
		ylabel('Avg Touch A unblocked prefoll and Touch A unblocked firstfoll RB switches','FontSize',9)
	...
	if a>=1 && a<=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchAunblockedprefollVSTouchAunblockedfirstfollRB_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchAunblockedprefollVSTouchAunblockedfirstfollRB_TimeRed_ObjectiveLeftAlignedtoTargetOnset.pdf'))
	end
