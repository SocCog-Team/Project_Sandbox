...
function [DistGazeATouchA, DistGazeATouchB]= tn_TrialWiseDISTNEWPlotsAlignedtoBIFR(distGazeATouchA, distGazeATouchB,InterpepochdataRegGazeB_Initial_Fixation_Release_A_poly_right, ModifiedTrialSets, saving_dir, )
close all
...
%
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwGazeTouchTime_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
    end
end
