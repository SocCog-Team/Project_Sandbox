function [] = rn_PlotAvgCombinedAAunblockedVSAAblocked_switch_BR (FullStructure,FullStructure_unblocked_pre,FullStructure_unblocked_first,FullStructure_unblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1 : 8
   disp(['Processing condition: ', FullStructure_unblocked_pre_AA{a}]);
	cur_fieldnames_unblocked_first_AA = FullStructure_unblocked_pre_AA{a};
	
    
	if (a==2) ||(a==6)
		cur_fieldnames_unblocked_AA_pre = FullStructure_unblocked_first_AA{a};
	else
		cur_fieldnames_unblocked_AA_pre = FullStructure_unblocked_first_AA{a};
	...
	if a>=1 && a<=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedSWITCH_BRTimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedSWITCH_BRTimeRed_ObjectiveLeftAlignedtoTargetOnset.pdf'))
	end
