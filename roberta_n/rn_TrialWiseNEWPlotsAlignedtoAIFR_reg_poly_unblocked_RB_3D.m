...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoAIFR_reg_poly_unblocked_RB_3D(epochdataRegisteredGazeA_poly_right, epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)

Byrewardttype_pos_unblocked_RB_Names=fieldnames(ModifiedTrialSets.Byrewardttype_pos.ByPostSwitch.Unblocked.RB);
...
%     end
    ...
    AverageTouchA_x = mean(epochdataTouchA.xCoordinates(TrialLists,:),1,'omitnan');
    AverageTouchA_y = mean(epochdataTouchA.yCoordinates(TrialLists,:),1,'omitnan');
    AverageTouchB_x = mean(epochdataTouchB.xCoordinates(TrialLists,:),1,'omitnan');
    AverageTouchB_y = mean(epochdataTouchB.yCoordinates(TrialLists,:),1,'omitnan');
    AverageGazeA_x = mean(epochdataRegisteredGazeA_poly_right.xCoordinates(TrialLists,:),1,'omitnan');
    AverageGazeA_y = mean(epochdataRegisteredGazeA_poly_right.yCoordinates(TrialLists,:),1,'omitnan');
    
    %   av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
    [a b ]= size(vergence);
    v(1:b) = 400;
    OverLayPlotsTouchA=plot3(epochdataTouchA.timepoints(TrialLists,:),epochdataTouchA.xCoordinates(TrialLists,:),epochdataTouchA.yCoordinates(TrialLists,:),'.','Color','r','MarkerSize', 4);
    ...
    hold on
    OverLayPlotsTouchB=plot3(epochdataTouchA.timepoints(TrialLists,:),epochdataTouchB.xCoordinates(TrialLists,:),epochdataTouchB.yCoordinates(TrialLists,:),'.','Color','b','MarkerSize', 4);
    ...
    hold on
    OverLayPlotsGazeA=plot3( epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.xCoordinates(TrialLists,:),...
                             epochdataRegisteredGazeA_poly_right.yCoordinates(TrialLists,:),'.','Color','k','MarkerSize', 4);
    ...
    %   OverLayPlotsGazeA.Color='red';
     ...
     hold on
     OverLayPlotsVergence = plot3(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),NewAverageVergence,v,'.','Color','m' )
    ...
    if counter>=1 && counter<=4
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveRightTop.pdf']))
        
    end
    if counter>=5 && counter<=8
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveRightCenter.pdf']))
        
    end
    if counter>=9 && counter<=12
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveRightBottom.pdf']))
        
    end
    if counter>=13 && counter<=16
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveLeftTop.pdf']))
        
    end
    if counter>=17 && counter<=20
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveLeftCenter.pdf']))
        
    end
    if counter>=21 && counter<=24
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_RB_Red_ObjectiveLeftBottom.pdf']))
    end
