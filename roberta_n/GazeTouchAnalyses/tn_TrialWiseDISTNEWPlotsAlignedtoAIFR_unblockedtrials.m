...
function [DistGazeATouchA, DistGazeATouchB]= tn_TrialWiseDISTNEWPlotsAlignedtoAIFR_unblockedtrials(distGazeATouchA, distGazeATouchB,InterpepochdataRegGazeA_Initial_Fixation_Release_A_poly_right, ModifiedTrialSets, saving_dir, fileID)
close all
...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
	...
	DistGazeATouchAOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),mean(distGazeATouchA(TrialLists,:)),'*','Color','r');
	hold on
	DistGazeATouchBOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),mean(distGazeATouchB(TrialLists,:)),'*','Color','b');
	hold on
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveRightTopAlignedtoAIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveRightCenterAlignedtoAIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveRightBottomAlignedtoAIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveLeftTopAlignedtoAIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveLeftCenterAlignedtoAIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_UnBlocked_Red_ObjectiveLeftBottomAlignedtoAIFR.jpg']))
    end
end
