...
function [DistGazeATouchA, DistGazeATouchB]= tn_TrialWiseDISTNEWPlotsAlignedtoBIFR_unlockedtrials_RB(distGazeATouchA, distGazeATouchB,InterpepochdataRegGazeB_Initial_Fixation_Release_A_poly_right, ModifiedTrialSets, saving_dir, fileID)
close all
...
%
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_ObjectiveLeftTopAlignedtoBIFR.pdf']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_ObjectiveLeftCenterAlignedtoBIFR.pdf']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwGazeTouchTime_Unblocked_RB_ObjectiveLeftBottomAlignedtoBIFR.pdf']))
    end
end
