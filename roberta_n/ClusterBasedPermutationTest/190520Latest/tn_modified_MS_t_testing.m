function [ scores ] = tn_modified_MS_t_testing(DistAA, DistAB, keys)
...
cr_value = 2.576;%ttest critical value for 0.005 p
...
for fr=1:size(keys, 2),
	...
	switch test_type
		...
		case 'ttest2'
			[h, p, ci, stats] = ttest2(DistAA(finite_AAAB_idx, fr), DistAB(finite_AAAB_idx, fr)); % h=1 - reject the null hypothesis sm: this is a paired t-test		
	end
...
for prm = 1 : num_perm,
	...
	for clus = 1:numel(scores.clusters),
		...
		if numel(scores.clusters{1, clus}) == 1,		
			finite_AA_idx = find(~isnan(DistAA(:, scores.clusters{clus})));
			finite_AB_idx = find(~isnan(DistAB(:, scores.clusters{clus})));
			finite_AAAB_idx = intersect(finite_AA_idx, finite_AB_idx);
			...
			switch test_type
				...
				case 'ttest2'
					[h, p, ci, stats] = ttest2(pseudo_AA_data(finite_AAAB_idx, scores.clusters{clus}), ...
						pseudo_AB_data(finite_AAAB_idx, scores.clusters{clus}));
			end			
...
% 				end
				...
				switch test_type
					...
					case 'ttest2'
						[h, p, ci, stats] = ttest2(pseudo_AA_data(finite_AAAB_idx, frv), ...
					pseudo_AB_data(finite_AAAB_idx, frv));
				end
