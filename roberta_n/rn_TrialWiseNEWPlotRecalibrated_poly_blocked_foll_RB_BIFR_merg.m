...
function [TouchA ,TouchB, GazeA, Vergence]= rn_TrialWiseNEWPlotRecalibrated_poly_blocked_foll_RB_BIFR_merg(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)
...
for counter = 1:8
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_BIFR_Blocked_FirstFollowing_RB_Red_ObjectiveRight.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_BIFR_Blocked_FirstFollowing_RB_Red_ObjectiveLeft.pdf'))
