...
function [DistGazeATouchA, DistGazeATouchB]= rn_TrialWiseDISTNEWPlotsAlignedOnset_blockedtrials_RB(distGazeATouchA, distGazeATouchB,epochdataRegisteredGazeA_poly_right, ModifiedTrialSets, saving_dir, fileID)
close all
...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
	...
	AbsDistGazeATouchAAVGOverlay=plot(epochdataGazeA.TargetOnset.timepoints(TrialLists,:),mean(distGazeATouchA(TrialLists,:)),'*','Color','r');
	hold on
	AbsDistGazeATouchBAVGOverlay=plot(epochdataGazeA.TargetOnset.timepoints(TrialLists,:),mean(distGazeATouchB(TrialLists,:)),'*','Color','b');
	hold on
...
%
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_Blocked_RB_Red_ObjectiveRightTopAlignedtoOnset.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoOnset.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveRightBottomAlignedtoOnset.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveLeftTopAlignedtoOnset.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveLeftCenterAlignedtoOnset.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoOnset.jpg']))
    end
end
