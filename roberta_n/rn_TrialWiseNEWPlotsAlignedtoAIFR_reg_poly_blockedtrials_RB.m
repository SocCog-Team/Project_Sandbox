...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoAIFR_reg_poly_blockedtrials_RB(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)
...
for counter= 1:24
	...
	if counter>=1 && counter<=4
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_BLocked_RB_Red_ObjectiveRightTopAlignedtoAIFR.jpg']))
		
	end
	if counter>=5 && counter<=8
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoAIFR.jpg']))
		
	end
	if counter>=9 && counter<=12
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottomAlignedtoAIFR.jpg']))
		
	end
	if counter>=13 && counter<=16
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTopAlignedtoAIFR.jpg']))
		
	end
	if counter>=17 && counter<=20
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenterAlignedtoAIFR.jpg']))
		
	end
	if counter>=21 && counter<=24
		saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoAIFR.jpg']))
	end
end
...
%     min(nnz(~isnan(epochdataTouchB.A_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))))
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottomAlignedtoAIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoAIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightTopAlignedtoAIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoAIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenterAlignedtoAIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTopAlignedtoAIFR.jpg']))
    end
end
