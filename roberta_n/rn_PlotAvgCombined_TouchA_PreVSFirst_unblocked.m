function [] = rn_PlotAvgCombined_TouchA_PreVSFirst_unblocked (FullStructure, FullStructure_unblocked_pre, FullStructure_unblocked_first, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_unblockedPreVSTouchAunblocked_Firstmergedconditions_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_unblockedPreVSTouchAunblocked_Firstmergedconditions_TimeRed_ObjectiveLefttAlignedtoTargetOnset.pdf'))
	end
