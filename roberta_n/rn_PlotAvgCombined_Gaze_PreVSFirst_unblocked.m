function [] = rn_PlotAvgCombined_Gaze_PreVSFirst_unblocked (FullStructure, FullStructure_unblocked_pre, FullStructure_unblocked_first, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1 : 8
	...
	if (a==3) ||(a==7)
		cur_fieldnames_unblocked_A_newconditon = FullStructure_unblocked_first_A{a+1};
	else
	    cur_fieldnames_unblocked_A_newconditon = FullStructure_unblocked_first_A{a};

	end
	
	TrialLists_unblocked_A = FullStructure_unblocked_pre.A.(cur_fieldnames_unblocked_pre_A);
	tmp_TrialLists_unblocked_A = FullStructure_unblocked_first.A.(cur_fieldnames_unblocked_A_newconditon);
...
% 	end
	...
	if b==1
		ylabel('Avg Gaze unblocked and Gaze unblocked','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgGaze_unblockedPreVSGazeunblocked_Firstmergedconditions_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgGaze_unblockedPreVSGazeunblocked_Firstmergedconditions_TimeRed_ObjectiveLefttAlignedtoTargetOnset.pdf'))
	end
