...
function []= rn_TrialWiseNEWPlotRecalibrated_poly(epochdataRegisteredGazeA_poly, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)
...
%     end
    ...
    AverageTouchA = mean(epochdataRegisteredGazeA_poly.TargetOnset.xCoordinates(TrialLists,:),1,'omitnan');
    AverageTouchB = mean(epochdataRegisteredGazeA_poly.TargetOnset.xCoordinates(TrialLists,:),1,'omitnan');
	
%   av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
   
    
    AverageGazeA=mean(epochdataRegisteredGazeA_poly.TargetOnset.xCoordinates(TrialLists,:),1,'omitnan');
    ...
    OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly.TargetOnset.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly.TargetOnset.xCoordinates(TrialLists,:),'.','Color','k','MarkerSize', 4);
    hold on
    OverLayPlotsGazeAVGA=plot(epochdataRegisteredGazeA_poly.TargetOnset.timepoints(TrialLists(1),:),AverageGazeA,'.','Color',[1,0.5,0],'MarkerSize', 6 )
    hold on
...
%     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveRightTop.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveRightCenter.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveRightBottom.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveLeftTop.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveLeftCenter.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTimeRed_ObjectiveLeftBottom.jpg']))
    end
end
