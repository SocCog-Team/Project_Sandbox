...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A, Timepoints]= rn_TrialWiseDISTNEWPlotsOnset_switchBR_prefoll_blocked_mer(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)

Byrewardttype_pos_blocked_Names=fieldnames(ModifiedTrialSets.ByStimulusPosition_leftrightChoicecategories.BySwitchingBlock.Prefirstfollowing.Blocked.BR);
...
for counter= 1:8
	...
	Byrewardttype_posSpStr = ModifiedTrialSets.ByStimulusPosition_leftrightChoicecategories.BySwitchingBlock.Prefirstfollowing.Blocked.BR.(Byrewardttype_posSp{1});
...
if counter==1
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_blocked_prefirstfollowing_RB_Red_ObjectiveRightAlignedtoOnset.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_blocked_prefirstfollowing_BR_Red_ObjectiveLeftAlignedtoOnset.pdf'))
