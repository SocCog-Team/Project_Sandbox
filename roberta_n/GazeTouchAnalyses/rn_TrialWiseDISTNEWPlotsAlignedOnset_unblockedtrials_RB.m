...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
  xlim([-0.5 1.8])
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveRightTopAlignedtoOnset.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveRightCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveRightBottomAlignedtoOnset.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveLeftTopAlignedtoOnset.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveLeftCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveLeftBottomAlignedtoOnset.pdf'))
    end
end
