...
function [DistGazeATouchA, DistGazeATouchB]= tn_TrialWiseDISTNEWPlotsAlignedtoBIFR(distGazeATouchA, distGazeATouchB,epochdataGazeA, ModifiedTrialSets, saving_dir, fileID)
close all
...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
    ...
    AbsDistGazeATouchAOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),distGazeATouchA(TrialLists,:),'.','Color','r');
    hold on
    AbsDistGazeATouchBOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),distGazeATouchB(TrialLists,:),'.','Color','b');
    hold on
    
%     AbsDistGazeATouchAAVGOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),mean(distGazeATouchA(TrialLists,:)),'O','Color','r');
%     hold on
%     AbsDistGazeATouchBAVGOverlay=plot(epochdataGazeA.timepoints(TrialLists,:),mean(distGazeATouchB(TrialLists,:)),'O','Color','b');
%     hold on
    xlim([-0.2 0.9])
    yL = get(gca,'YLim');    
    hold on
...
%
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveRightTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveRightCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveRightBottomAlignedtoBIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveLeftTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveLeftCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbsDiffbwGazeTouchTimeRed_ObjectiveLeftBottomAlignedtoBIFR.jpg']))
    end
end
