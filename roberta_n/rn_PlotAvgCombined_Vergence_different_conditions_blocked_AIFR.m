function [] = rn_PlotAvgCombined_Vergence_different_conditions_blocked_AIFR (FullStructure, FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if b==1
		ylabel('Avg Vergence blocked and Vergence blocked AIFR','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoAIFR.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoAIFR.pdf'))
	end
	...
	%     if a>=9 && a<=12
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveRightBottomAlignedtoAIFR.jpg']))
	%
	%     end
	%     if a>=13 && a<=16
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftTopAlignedtoAIFR.jpg']))
	%
	%     end
	%     if a>=17 && a<=20
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftCenterAlignedtoAIFR.jpg']))
	%
	%     end
	%     if a>=21 && a<=24
	%         saveas(gcf, fullfile(saving_dir,[fileID, '_AvgDiffbwVergenceTouchTimepSeeLess50Red_ObjectiveLeftBottomAlignedtoAIFR.jpg']))
	%     end
