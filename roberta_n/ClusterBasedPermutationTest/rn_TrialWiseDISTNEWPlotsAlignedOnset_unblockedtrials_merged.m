...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A, Timepoints]= rn_TrialWiseDISTNEWPlotsAlignedOnset_unblockedtrials_merged(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)

Byrewardttype_pos_unblocked_Names=fieldnames(ModifiedTrialSets.ByStimulusPosition_leftrightChoicecategories.SuccessfulChoiceTrialsUnblocked);
...
for counter= 1:8
	...
	Byrewardttype_posSpStr=ModifiedTrialSets.ByStimulusPosition_leftrightChoicecategories.SuccessfulChoiceTrialsUnblocked.(Byrewardttype_posSp{1});
	...
	if counter==1
		...
		Timepoints.TrialWise.Type2401 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	end
	...
	if counter==2
	    ...
	    Timepoints.TrialWise.Type2402 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==3
		...
		Timepoints.TrialWise.Type2403 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==4
		...
		Timepoints.TrialWise.Type2404 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==5
		...
		Timepoints.TrialWise.Type2405 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==6
		...
		Timepoints.TrialWise.Type2406 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==7
		...
		Timepoints.TrialWise.Type2407 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter==8
	    ...
	    Timepoints.TrialWise.Type2408 = epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:);
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Unblocked_Red_ObjectiveRightAlignedtoOnset.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Unblocked_Red_ObjectiveLeftAlignedtoOnset.pdf'))
