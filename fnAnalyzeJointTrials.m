	...
	if (PlotRTHistograms)
		for i_histogram_RT_type = 1 : length(histogram_RT_type_list)
           ...
           if (plot_RT_A_vs_B)
				...
				%choice or B's choice
				own_choice_A_index = find(SameOwnA_lidx);
				own_choice_B_index = find(SameOwnB_lidx);
				
				%reaction time list for all trials in which both player's
				%seected A's preffered option for player A & B
				same_choice_A_preferred_A = A_TargetAcquisitionRT(intersect(GoodTrialsIdx,own_choice_A_index));
				same_choice_A_preferred_B = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,own_choice_A_index));
				
				%reaction time list for all trials where both players
				%selected B's prefered color
				same_choice_B_preferred_A = A_TargetAcquisitionRT (intersect(GoodTrialsIdx,own_choice_B_index));
				same_choice_B_preferred_B = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,own_choice_B_index));
				%Create scatter plot for corrdinated choices 
				scatter(same_choice_A_preferred_A ,same_choice_A_preferred_B ,10, 'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[1 0 0]);
				scatter(same_choice_B_preferred_A ,same_choice_B_preferred_B , 10,'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[0 0 1]);
				%gather indexes of different own choices and different
				%other choices
				diff_choice_own_index = find (DiffOwn_lidx);
				diff_choice_other_index = find (DiffOther_lidx);
				%reaction times for different own choices for A & B
				diff_choice_own_A = A_TargetAcquisitionRT  (intersect(GoodTrialsIdx,diff_choice_own_index));
				diff_choice_own_B = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,diff_choice_own_index));
				%reaction times for different other choices for A & B
				diff_choice_other_A = A_TargetAcquisitionRT (intersect(GoodTrialsIdx,diff_choice_other_index));
				diff_choice_other_B = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,diff_choice_other_index));
				%create scatter plot for uncoordinated choices
				
		
				scatter(diff_choice_own_A,diff_choice_own_B, 10, 'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[1 0 1]);
				scatter(diff_choice_other_A,diff_choice_other_B,10, 'MarkerEdgeColor',[0 0 0],...
...
%  				ax.XTick = 0:100:max(A_TargetAcquisitionRT);
%  				ax.YTick = 0:100:max(A_TargetAcquisitionRT);
%                 axis equal ;
				xlabel('RT_A','FontSize',10,'FontWeight','bold');
				ylabel('RT_B','FontSize',10,'FontWeight','bold');
				legend('Same_own_A','Same_own_B', 'Diff_Own', 'Diff_other');
				...
				% add this as title
				df_corr = length(JointTrialX_Vector) - 2;
				titleText = {[['TargetAcquisition time correlation same choice A: r(', num2str(df_corr), '): ', num2str(TAcq_r_same_choice_A(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_same_choice_A(2, 1)),]]...
					[['TargetAcquisition time correlation same choice B: r(', num2str(df_corr), '): ', num2str(TAcq_r_same_choice_B(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_same_choice_B(2, 1)),]]...
					[['TargetAcquisition time correlation different own: r(', num2str(df_corr), '): ', num2str(TAcq_r_diff_own_choice(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_diff_own_choice(2, 1)),]]...
					[['TargetAcquisition time correlation different other: r(', num2str(df_corr), '): ', num2str(TAcq_r_diff_other_choice(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_diff_other_choice(2, 1)),]]};
				...
				A_other_index = (PreferableTargetSelected_A == 0);
				
				A_own_choice_trials_index = find(A_own_index);
				A_other_choice_trials_index = find(A_other_index);
				...
				%A_all_choice_index = union (A_own_choice_trials_index,A_other_choice_trials_index);
% 				A_own_target_vector = intersect(GoodTrialsIdx, A_own_choice_trials_index);
% 				A_other_target_vector = intersect (GoodTrialsIdx, A_other_choice_trials_index);
				A_own_choice = A_TargetAcquisitionRT (intersect(GoodTrialsIdx,A_own_choice_trials_index ));
				A_other_choice = A_TargetAcquisitionRT (intersect(GoodTrialsIdx,A_other_choice_trials_index ));
				%axis([0 max(TrialSets.All) 0	max(A_TargetAcquisitionRT)]); 
				scatter (B_choice_when_A_own_trials,A_own_choice,10, 'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[1 0 0], 'LineWidth',1);
				
				hold on
				
				scatter (B_choice_when_A_other_trials, A_other_choice, 10, 'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[0 0 1]);
...
%                 axis equal ;
				...
				legend('A-Own','A-other');
				
				[TAcq_r_own_A, TAcq_p_own_A] = corrcoef(A_own_choice,B_choice_when_A_own_trials);
				[TAcq_r_other_A, TAcq_p_other_A] = corrcoef( A_other_choice,B_choice_when_A_other_trials);
				titleText_A_choices = {[['TargetAcquisition time correlation own choice A: r(', num2str(df_corr), '): ', num2str(TAcq_r_own_A(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_own_A(2, 1)),]]...
					[['TargetAcquisition time correlation other choice A: r(', num2str(df_corr), '): ', num2str(TAcq_r_other_A(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_other_A(2, 1)),]]}
				title(titleText_A_choices, 'FontSize', title_fontsize, 'Interpreter', 'None', 'FontWeight', title_fontweight);	
				hold off
				
				subplot (1,3,3);
				B_own_index = (PreferableTargetSelected_B == 1);
				B_other_index = (PreferableTargetSelected_B == 0);
				
				B_own_choice_trials_index = find(B_own_index);
				B_other_choice_trials_index = find(B_other_index);
				
				A_choice_when_B_own_trials = A_TargetAcquisitionRT(intersect(GoodTrialsIdx,	B_own_choice_trials_index));
				A_choice_when_B_other_trials = A_TargetAcquisitionRT(intersect(GoodTrialsIdx,B_other_choice_trials_index));
				%A_all_choice_index = union (A_own_choice_trials_index,A_other_choice_trials_index);
% 				B_own_target_vector = intersect(GoodTrialsIdx, B_own_choice_trials_index);
% 				B_other_target_vector = intersect (GoodTrialsIdx, B_other_choice_trials_index);
				B_own_choice = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,B_own_choice_trials_index ));
				B_other_choice = B_TargetAcquisitionRT (intersect(GoodTrialsIdx,B_other_choice_trials_index ));
				%axis([0 max(TrialSets.All) 0 max(A_TargetAcquisitionRT)]);
				scatter (A_choice_when_B_own_trials,B_own_choice,10, 'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[0 0 1]);
				
				hold on
				
				scatter (A_choice_when_B_other_trials, B_other_choice, 10, 'MarkerEdgeColor',[0 .5 .5],...
					'MarkerFaceColor',[1 0 0]);
				
				legend ('B-own', 'B-other');
				axis equal ;
				ax=gca;
				%axis square;
...
%                 axis equal ;
				...
				[TAcq_r_own_B, TAcq_p_own_B] = corrcoef(B_own_choice,A_choice_when_B_own_trials);
				[TAcq_r_other_B, TAcq_p_other_B] = corrcoef( B_other_choice,A_choice_when_B_other_trials);
				titleText_B_choices = {[['TargetAcquisition time correlation own choice B: r(', num2str(df_corr), '): ', num2str(TAcq_r_own_B(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_own_B(2, 1)),]]...
					[['TargetAcquisition time correlation other choice B: r(', num2str(df_corr), '): ', num2str(TAcq_r_other_B(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_other_B(2, 1)),]]}
				title(titleText_B_choices, 'FontSize', title_fontsize, 'Interpreter', 'None', 'FontWeight', title_fontweight);	
				hold off
				
				figure('Name', 'RT_A_vs_B_by_side')
				%plot side based RT for both subjects
				subplot(1,3,1)
				A_left_index = find(SubjectiveLeftTargetSelected_A);
				A_right_index = find (SubjectiveRightTargetSelected_A);
				B_left_index = find (SubjectiveLeftTargetSelected_B);
				B_right_index = find(SubjectiveRightTargetSelected_B);
				
				same_side_A_left_B_left = SubjectiveLeftTargetSelected_A & SubjectiveLeftTargetSelected_B;
				diff_side_A_left_B_right =SubjectiveLeftTargetSelected_A & SubjectiveRightTargetSelected_B;
				diff_side_A_right_B_left = SubjectiveRightTargetSelected_A & SubjectiveLeftTargetSelected_B;
				same_side_A_right_B_right = SubjectiveRightTargetSelected_A & SubjectiveRightTargetSelected_B;
				
				same_side_A_left_B_left_index = find(same_side_A_left_B_left);
				diff_side_A_left_B_right_index = find(diff_side_A_left_B_right);
				diff_side_A_right_B_left_index = find(diff_side_A_right_B_left);
				same_side_A_right_B_right_index = find(same_side_A_right_B_right);
				
				
% 				Good_trials_A_left_index = intersect(GoodTrialsIdx, A_left_index);
% 				Good_trials_A_right_index = intersect(GoodTrialsIdx, A_right_index);
% 				Good_trials_B_left_index = intersect(GoodTrialsIdx, B_left_index);
% 				Good_trials_B_right_index = intersect(GoodTrialsIdx, B_right_index);
% 				
                same_choice_A_left_B_left_RT_A = A_TargetAcquisitionRT(intersect(same_side_A_left_B_left_index,GoodTrialsIdx));
				same_choice_A_left_B_left_RT_B = B_TargetAcquisitionRT(intersect(same_side_A_left_B_left_index,GoodTrialsIdx));
				
				same_choice_A_right_B_right_RT_A = A_TargetAcquisitionRT(intersect(same_side_A_right_B_right_index,GoodTrialsIdx));
				same_choice_A_right_B_right_RT_B = B_TargetAcquisitionRT(intersect(same_side_A_right_B_right_index,GoodTrialsIdx));
				
				diff_choice_A_left_B_right_RT_A= A_TargetAcquisitionRT(intersect(diff_side_A_left_B_right_index,GoodTrialsIdx));
				diff_choice_A_left_B_right_RT_B = B_TargetAcquisitionRT(intersect(diff_side_A_left_B_right_index,GoodTrialsIdx));
				
				diff_choice_A_right_B_left_RT_A = A_TargetAcquisitionRT(intersect(diff_side_A_right_B_left_index,GoodTrialsIdx));
				diff_choice_A_right_B_left_RT_B = B_TargetAcquisitionRT(intersect(diff_side_A_right_B_left_index,GoodTrialsIdx));
			
				
				hold on
				
				scatter ( same_choice_A_left_B_left_RT_A ,same_choice_A_left_B_left_RT_B,10,  'MarkerEdgeColor',[0 0 0],...
					'MarkerFaceColor',[1 0 0]);
...
% 				
				...
				[TAcq_r_side_A_left_B_left, TAcq_p_side_A_left_B_left] = corrcoef( same_choice_A_left_B_left_RT_A,same_choice_A_left_B_left_RT_B);
				...
				[TAcq_r_side_A_right_B_left, TAcq_p_side_A_right_B_left] = corrcoef(diff_choice_A_right_B_left_RT_A,diff_choice_A_right_B_left_RT_B);
				titleText_choice_by_side = {[['TargetAcquisition time correlation A-left-B-left: r(', num2str(df_corr), '): ', num2str(TAcq_r_side_A_left_B_left(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_side_A_left_B_left(2, 1)),]]...
					[['TargetAcquisition time correlation A-right-B-right: r(', num2str(df_corr), '): ', num2str(TAcq_r_side_A_right_B_right(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_side_A_right_B_right(2, 1)),]]...
					[['TargetAcquisition time correlation A-left-B-right: r(', num2str(df_corr), '): ', num2str(TAcq_r_side_A_left_B_right(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_side_A_left_B_right(2, 1)),]]...
					[['TargetAcquisition time correlation A-right-B-left: r(', num2str(df_corr), '): ', num2str(TAcq_r_side_A_right_B_left(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_side_A_right_B_left(2, 1)),]]}
				title(titleText_choice_by_side, 'FontSize', title_fontsize, 'Interpreter', 'None', 'FontWeight', title_fontweight);	
				hold off
...
% 				
% 				[TAcq_r_side_A_right_B_left, TAcq_p_side_A_right_B_left] = corrcoef(diff_choice_A_right_B_left_RT_A,diff_choice_A_right_B_left_RT_B);
% 				titleText_A_right_B_left = {[['TargetAcquisition time correlation A-right-B-left: r(', num2str(df_corr), '): ', num2str(TAcq_r_side_A_right_B_left(2, 1), '%.4f'),  ', p <= ', num2str(TAcq_p_side_A_right_B_left(2, 1)),]]}
% 				title(titleText_A_right_B_left, 'FontSize', title_fontsize, 'Interpreter', 'None', 'FontWeight', title_fontweight);	
% 				hold off
% 				saveas(gcf,'RT_A_vs_B.pdf');
% 				savepath (OutputPath);

				figure('Name', 'Linear Fit')
				subplot(2,1,1)
				
				if isempty(same_choice_A_preferred_A) & isempty(same_choice_B_preferred_B)
				'solo trial'
				else
				...
				elseif isempty(same_choice_B_preferred_A & same_choice_B_preferred_B)
				...
				subplot (2,1,2)
				if isempty(diff_choice_own_A) & isempty(diff_choice_own_B)
					'solo trial'
				else
					...
					elseif isempty(diff_choice_own_A & diff_choice_own_B)
                ...
                outfile_fqn = fullfile(OutputPath, [FileName, '.RT.A_vs_B.', OutPutType]);
				write_out_figure(Cur_fh_ReactionTimesBySameness, outfile_fqn);
