function [] = fn_touch_and_gaze_analysis_Elmo_JK(fileID, gazereg_name)
...
%apply the chosen registration to the raw left and right eye (all the trials)
...
recalibration_struct.header = {'TrialNumber', 'CorrectedTimeStamps', 'Left_Eye_x', 'Left_Eye_y', 'Right_Eye_x', 'Right_Eye_y'};
recalibration_struct.cn = local_get_column_name_indices(recalibration_struct.header); %struct(field1,value1,field2,value2,field3,value3,field4,value4, field5, value5, field6, value6);
recalibration_struct.data = horzcat (data_struct_extract.data(:, data_struct_extract.cn.TrialNumber), data_struct_extract.data(:,data_struct_extract.cn.Tracker_corrected_EventIDE_TimeStamp) ,...
	registered_left_eye_gaze_samples(:,1),registered_left_eye_gaze_samples(:,2),registered_right_eye_gaze_samples(:,1),registered_right_eye_gaze_samples(:,2));
...
rn_TrialWiseNEWPlotRecalibrated_poly_blockedtrials(InterpepochdataRegGazeA_poly_right,InterpepochdataRegGazeA_poly_left, InterpolatedepochdataTouchA,InterpolatedepochdataTouchB,ModifiedTrialSets, saving_dir, fileID);
