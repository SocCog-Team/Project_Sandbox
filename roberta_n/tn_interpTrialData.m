...
for i = 1:a
    if ~isnan(TrialWiseData.timepoints(i,1))
        ...
        [r, LastTimepoint_idx] = find(TrialWiseData.timepoints(i, :) == LastTimepoint); %max(TrialWiseData.timepoints(i,:)));
		...
		tmp_per_trial_timestamp_grid = (TrialWiseData.timepoints(i,1):2:LastTimepoint);
        InterpolatedTrialWiseData.timepoints(i, 1:length(tmp_per_trial_timestamp_grid)) = tmp_per_trial_timestamp_grid;
        lengthOfInterpolated = length(tmp_per_trial_timestamp_grid);
