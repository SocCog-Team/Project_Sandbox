function [] = rn_PlotAvgCombined_Vergence_different_conditions_blocked (FullStructure, FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if b==1
		ylabel('Avg Vergence blocked and Vergence blocked','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergence_blockedVSVergenceblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoTargetOnset.pdf'))
	end
