...
function [DistGazeATouchA, DistGazeATouchB, TimepointsTrialWise, BIFRAlignedAIFRAVG]= tn_TrialWiseDISTNEWPlotsAlignedtoBIFRpSeeLesserThanChance(distGazeATouchA, distGazeATouchB,epochdataGazeATimepoints, ModifiedTrialSets, saving_dir, fileID, AIFRrelativetoBIFR)
...
for counter= 1:length(pSeeLesserThanChanceNames)
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveRightTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveRightCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveRightBottomAlignedtoBIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveLeftTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveLeftCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XAbspSeeLessThanChanceDiffbwGazeTouchTimeRed_ObjectiveLeftBottomAlignedtoBIFR.jpg']))
    end
    xlim([-0.6 0.9])
    ylim([-500 500])
end
