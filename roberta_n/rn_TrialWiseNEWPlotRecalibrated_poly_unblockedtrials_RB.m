...
%   av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
    ...
    hold on
    OverLayPlotsTouchAVGA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists(1),:),AverageTouchA,'.','Color','r','MarkerSize', 8)
    hold on
    OverLayPlotsTouchAVGB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists(1),:),AverageTouchB,'.','Color','b','MarkerSize',8)
    
    OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.TargetOnset.xCoordinates(TrialLists,:),'.','Color','k','MarkerSize', 4);
    hold on
...
%     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
   ...
   hold on 
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightTop.pdf'))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightCenter.pdf'))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightBottom.pdf'))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftTop.pdf'))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftCenter.pdf'))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftBottom.pdf'))
    end
end
...
%     end
%     if counter>=1 && counter<=4 
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightTop.pdf'))
%         
%     end
%     if counter>=5 && counter<=8 
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightCenter.pdf'))
%         
%     end
%     if counter>=9 && counter<=12  
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightBottom.pdf'))
%         
%     end
%     if counter>=13 && counter<=16 
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftTop.pdf'))
%         
%     end
%     if counter>=17 && counter<=20  
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftCenter.pdf'))
%         
%     end
%     if counter>=21 && counter<=24 
%         saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftBottom.pdf'))
%     end
end