...
function []= rn_TrialWiseNEWPlotRecalibrated_poly(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)
...
%     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
   ...
   hold on 
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Red_ObjectiveRightTop.pdf'))
       
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Red_ObjectiveRightCenter.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Red_ObjectiveRightBottom.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Red_ObjectiveLeftTop.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Red_ObjectiveLeftCenter.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveLeftBottom.pdf'))
    end
end
...
% % av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
    ...
    OverLayPlotsTouchA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),epochdataTouchA.TargetOnset.yCoordinates(TrialLists,:),'.','Color','r','MarkerSize', 4);
    hold on
    OverLayPlotsTouchB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists,:),epochdataTouchB.TargetOnset.yCoordinates(TrialLists,:),'.','Color','b','MarkerSize', 4);
    hold on
    OverLayPlotsTouchAVGA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists(1),:),AverageTouchA,'.','Color','r','MarkerSize', 6 )
    hold on
    OverLayPlotsTouchAVGB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists(1),:),AverageTouchB,'.','Color','b','MarkerSize', 6  )
...
%     OverLayPlotsTouchAVGB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists(1),:),AverageTouchB,'*','Color','b' )
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Red_ObjectiveRightTop.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveRightCenter.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveRightBottom.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveLeftTop.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Red_ObjectiveLeftCenter.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Red_ObjectiveLeftBottom.pdf'))
    end
end