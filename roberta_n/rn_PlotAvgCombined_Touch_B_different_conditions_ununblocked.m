function [] = rn_PlotAvgCombined_Touch_B_different_conditions_unblocked (FullStructure, FullStructure_ununblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';

fieldnames_ununblocked_B = fieldnames(FullStructure_ununblocked.B);

for a = 1:8
	cur_fieldnames_ununblocked_B = fieldnames_ununblocked_B{a};
	if (a==1) || (a==3) || (a==5) || (a==7)
...
% 	str_unblocked = strcat(TrialNumStr, num2str(size((tmp_TrialLists_unblocked_A),1)));
% 	
% 	complete_str = strcat ('A-unblocked: ', str_unblocked, ' A-unblocked: ', str_unblocked)
% 	title (str_unblocked);
% 	
% 	if b==1
% 		strCond ='A-Own/B-Own';
...
% 	end
	...
	%     [AClosertoZero BClosertoZero] = tn_ClosertoZerowithTarget(AvgAA_unblocked,AvgAA_unblocked,AvgAT_unblocked,AvgAT_unblocked,TrialListsTimepoints);
	hold on
	...
	%     hold on
	%     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked + SEM_AT_unblocked,'Color','k', 'LineWidth',0.5);
	%     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked - SEM_AT_unblocked,'Color','k', 'LineWidth',0.5);
	%     hold on
	% 	plot((TrialListsTimepoints(6,:)),AvgAT_unblocked,'Color','m', 'LineWidth',2);
	%     hold on
	%     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked + SEM_AT_unblocked,'Color','m', 'LineWidth',0.5);
	%     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked - SEM_AT_unblocked,'Color','m', 'LineWidth',0.5);
	%     hold on
	...
	%     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked - T_unblocked_CIHW95,'Color','k', 'LineWidth',0.5, 'LineStyle', '--');
	% %     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked + T_unblocked_CIHW95,'Color','m', 'LineWidth',0.5, 'LineStyle', '--');
	% %     plot((TrialListsTimepoints(6,:)),AvgAT_unblocked - T_unblocked_CIHW95,'Color','m', 'LineWidth',0.5, 'LineStyle', '--');
	...
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchB_unblockedVSTouchBunblocked_mergedconditions_TimeRed_ObjectiveLeftAlignedtoTargetOnset.pdf'))
	end
	
	% 	if ~(comparison_TargetOnset_AAunblockedVSAAunblocked) && ~(comparison_BIFR_AAunblockedVSAAunblocked)
	% 		if a>=1 && a<=4
	% 			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedTimeRed_ObjectiveRightAlignedtoAIFRwithTarget.pdf'))
	% 		end
	% 		if a>=5 && a<=8
	% 			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedTimeRed_ObjectiveLeftAlignedtoAIFRwithTarget.pdf'))
	% 		end
	% 	end
...
function [AClosertoZero, BClosertoZero]= tn_ClosertoZerowithTarget (SampleA, SampleB,SampleT_unblocked, Timepoints)
AClosertoZero = find((abs(SampleA(1,:))< abs(SampleB(1,:))) &(abs(SampleA(1,:))< abs(SampleT_unblocked(1,:))));
BClosertoZero = find((abs(SampleB(1,:))<=abs(SampleA(1,:))) & ((abs(SampleB(1,:))< abs(SampleT_unblocked(1,:)))));
...
% ClosertoZeroY(ClosertoZeroY==0)=280;
% ClosertoZeroY = transpose(ClosertoZeroY);
% plot(TunblockedClosertoZeroTimes,ClosertoZeroY,'.', 'Color','m');
end
