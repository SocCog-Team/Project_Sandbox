function [] = rn_PlotAvgCombined (FullStructure, FullStructure_unblocked,FullStructure_blocked,saving_dir)

comparison_TargetOnset_AAunblockedVSAAblocked = 0;
% comparison_BIFR_AAunblockedVSAAblocked = 0;
...
for a = 1:8
	...
	%     end
	%     str=strcat(TrialNumStr, num2str(size((TrialLists_unblocked_AA),1)));
	%     text(0.70,380,str);
	%     if b==1
	%         strCond ='A-Own/B-Own';
	...
	if  ~comparison_TargetOnset_AAunblockedVSAAblocked 
		if a>=1 && a<=4
			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveRightAlignedtoTargetOnsetwithTarget.pdf'))
		end
		if a>=5 && a<=8
			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveLeftAlignedtoTargetOnsetwithTarget.pdf'))
		end
	else
		if a>=1 && a<=4
			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveRightAlignedtoBIFRwithTarget.pdf'))
		end
		if a>=5 && a<=8
			write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveLeftAlignedtoBIFRwithTarget.pdf'))
		end
