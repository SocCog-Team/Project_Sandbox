...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A]= tn_TrialWiseDISTNEWPlotsAlignedtoBIFR_blockedtrials_RB(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
close all
...
for counter= 1:24
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
       
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwGazeTouchTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
    end
end
