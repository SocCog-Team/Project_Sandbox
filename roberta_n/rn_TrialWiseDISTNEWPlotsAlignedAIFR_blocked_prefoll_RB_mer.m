...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A, Timepoints]= rn_TrialWiseDISTNEWPlotsAlignedAIFR_blocked_prefoll_RB_mer(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
...
for counter= 1:8
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_prefirstfollowing_RB_Red_ObjectiveRightAlignedtoAIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_prefirstfollowing_RB_Red_ObjectiveLeftAlignedtoAIFR.pdf'))
