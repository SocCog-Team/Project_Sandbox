function [] = rn_PlotAvgCombined_Gaze_different_conditions_unblocked_AIFR (FullStructure, FullStructure_unblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if b==1
		ylabel('Avg Gaze unblocked and Gaze unblocked AIFR','FontSize',9)
	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgGaze_unblockedVSGazeunblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoAIFR.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgGaze_unblockedVSGazeunblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoAIFR.pdf'))
	end
