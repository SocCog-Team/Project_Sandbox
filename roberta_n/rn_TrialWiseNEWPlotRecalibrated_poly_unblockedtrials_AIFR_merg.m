function [TouchA ,TouchB, GazeA, Vergence]= rn_TrialWiseNEWPlotRecalibrated_poly_unblockedtrials_AIFR_merg(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)
...
for counter = 1:8
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_AIFR_Unlocked_Red_ObjectiveRight.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_AIFR_Unblocked_Red_ObjectiveLeft.pdf'))
...
% 	AverageTouchA=mean(epochdataTouchA.yCoordinates(TrialLists,:),1);
% 	AverageTouchB=mean(epochdataTouchB.yCoordinates(TrialLists,:),1);
