...
function []= rn_TrialWiseNEWPlotRecalibrated_poly_blockedtrials_RB(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)

Byrewardttype_pos_blocked_RB_Names=fieldnames(ModifiedTrialSets.Byrewardttype_pos.BySwitch_PostSwitching.Blocked.RB);
...
for counter = 1:24
    ...
    Byrewardttype_posSpStr = ModifiedTrialSets.Byrewardttype_pos.BySwitch_PostSwitching.Blocked.RB.(Byrewardttype_posSp{1});
...
%     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
   ...
   hold on 
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightTop.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenter.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottom.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTop.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenter.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottom.jpg']))
    end
end
...
% %     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottom.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenter.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightTop.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottom.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenter.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTop.jpg']))
    end
end