...
function [DistGazeATouchA, DistGazeATouchB, DistGazeA_finaltarget_A, DistTouchA_finaltarget_A, Timepoints]= rn_TrialWiseDISTNEWPlotsAlignedOnset_blockedtrials_merged(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
...
for counter= 1:8
	...
	DistGazeATouchAOverlay = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),distGazeATouchA(TrialLists,:),'.','Color','r','MarkerSize',2);
	hold on
	DistGazeATouchBOverlay = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),distGazeATouchB(TrialLists,:),'.','Color','b','MarkerSize',2);
	hold on
	DistGazeATouchAAVGOverlay = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AveragedistGazeATouchA,'*','Color','g','MarkerSize',2);
	hold on
	DistGazeATouchBAVGOverlay = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AveragedistGazeATouchB,'*','Color','y','MarkerSize',2);
	hold on
...
% 	hold on 
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_Red_ObjectiveRightAlignedtoOnset.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_Red_ObjectiveLeftAlignedtoOnset.pdf'))
