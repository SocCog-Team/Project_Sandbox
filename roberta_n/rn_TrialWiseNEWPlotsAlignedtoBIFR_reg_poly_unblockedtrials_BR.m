...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoBIFR_reg_poly_unblockedtrials_BR(epochdataRegisteredGazeA_poly_right, epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)
...
for counter= 1:24
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
		
	end
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
		
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
	end
end
...
% 	end
% 	if counter>=1 && counter<=4
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=5 && counter<=8
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=9 && counter<=12
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=13 && counter<=16
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=17 && counter<=20
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=21 && counter<=24
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
% 	end
% end
% 
% % %%SAVE THE FIGURES !
