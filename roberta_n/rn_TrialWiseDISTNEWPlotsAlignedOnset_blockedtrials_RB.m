...
% 	DistTouchB_finaltarget_BAVGOverlay = plot (epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AveragedistTouchB_finaltarget_B,'*','Color',[1 0.5 0 ],'MarkerSize',6);
	...
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveRightBottomAlignedtoOnset.pdf'))
		
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveLeftTopAlignedtoOnset.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_RB_Red_ObjectiveLeftCenterAlignedtoOnset.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoOnset.pdf'))
	end
end
