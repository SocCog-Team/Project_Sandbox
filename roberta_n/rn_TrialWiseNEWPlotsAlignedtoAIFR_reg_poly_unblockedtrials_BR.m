...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoAIFR_reg_poly_unblockedtrials_BR(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)
...
if counter>=1 && counter<=12
	...
	OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.xCoordinates(TrialLists,:),'.','Color','k', 'LineWidth',2);
	hold on
	OverLayPlotsTouchB=plot(epochdataTouchB.timepoints(TrialLists,:),epochdataTouchB.xCoordinates(TrialLists,:),'.','Color','b','LineWidth',2);
	hold on
	OverLayPlotsTouchA=plot(epochdataTouchA.timepoints(TrialLists,:),epochdataTouchA.xCoordinates(TrialLists,:),'.','Color','r', 'LineWidth',2);
	% %     OverLayPlotsGazeA.MarkerSize=3;
	%     OverLayPlotsGazeA.Color='red';
	hold on
	OverLayPlotsGazeAVGA=plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),AverageGazeA,'.','Color',[1,0.5,0],'LineWidth',2)
	hold on
	OverLayPlotsTouchAVGA=plot(epochdataTouchA.timepoints(TrialLists,:),AverageTouchA,'*','Color','g','LineWidth',2 );
	hold on
	OverLayPlotsTouchAVGB=plot(epochdataTouchB.timepoints(TrialLists,:),AverageTouchB,'*','Color','y','LineWidth',2);
	hold on
	yyaxis right
	OverLayPlotsVergence = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AverageVergence,'.','Color','m','LineWidth',2)
	set(gca,'ylim',[-50 50])
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBLocked_BR_Red_ObjectiveRightTopAlignedtoAIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightCenterAlignedtoAIFR.pdf'))
		
	end
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightBottomAlignedtoAIFR.pdf'))
		
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftTopAlignedtoAIFR.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftCenterAlignedtoAIFR.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftBottomAlignedtoAIFR.pdf'))
	end
end
...
% 	
% 	OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.yCoordinates(TrialLists,:),'.','Color','k','LineWidth',2);
% 	hold on
% 	OverLayPlotsTouchB=plot(epochdataTouchB.timepoints(TrialLists,:),epochdataTouchB.yCoordinates(TrialLists,:),'.','Color','b','LineWidth',2);
% 	hold on
% 	OverLayPlotsTouchA=plot(epochdataTouchA.timepoints(TrialLists,:),epochdataTouchA.yCoordinates(TrialLists,:),'.','Color','r', 'LineWidth',2);
% 	hold on
% 	OverLayPlotsGazeAVGA=plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AverageGazeA,'.','Color',[1 0.5 0],'LineWidth',2)
% 	hold on
% 	OverLayPlotsTouchAVGA=plot(epochdataTouchA.timepoints(TrialLists(1),:),AverageTouchA,'*','Color','g','LineWidth',2 )
% 	hold on
% 	OverLayPlotsTouchAVGB=plot(epochdataTouchB.timepoints(TrialLists(1),:),AverageTouchB,'*','Color','y','LineWidth',2 )
% 	
% 	
% 	%
% 	%     MaxLengthTimePoints_idx=find(epochdataTouchB.A_InitialFixationReleaseTime_ms.timepoints(TrialLists,:)==max(length(epochdataTouchB.A_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))));
...
% 	end
% 	if counter>=1 && counter<=4
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightTopAlignedtoAIFR.pdf'))
% 		
% 	end
% 	if counter>=5 && counter<=8
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightCenterAlignedtoAIFR.pdf'))
% 		
% 	end
% 	if counter>=9 && counter<=12
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveRightBottomAlignedtoAIFR.pdf'))
% 		
% 	end
% 	if counter>=13 && counter<=16
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftTopAlignedtoAIFR.pdf'))
% 		
% 	end
% 	if counter>=17 && counter<=20
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftCenterAlignedtoAIFR.pdf'))
% 		
% 	end
% 	if counter>=21 && counter<=24
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_BR_Red_ObjectiveLeftBottomAlignedtoAIFR.pdf'))
% 	end
% end
