...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoBIFR_reg_poly_blockedtrials_RB(epochdataRegisteredGazeA_poly_right, epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)

Byrewardttype_pos_blocked_RB_Names=fieldnames(ModifiedTrialSets.Byrewardttype_pos.BySwitchingBlock.Blocked.RB);
...
for counter= 1:24
    ...
    Byrewardttype_posSpStr=ModifiedTrialSets.Byrewardttype_pos.BySwitchingBlock.Blocked.RB.(Byrewardttype_posSp{1});
...
%     OverLayPlotsTouchAVGB=plot(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists(1),:),mean(epochdataTouchB.B_InitialFixationReleaseTime_ms.xCoordinates(TrialLists,:),1),'*','Color','b' )
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
    end
end
...
% 	hold on 
% 	yyaxis right
% 	OverLayPlotsVergence = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AverageVergence,'.','Color','m','MarkerSize',2)
% 	set(gca,'ylim',[-50 50])
%     
%     hold on 
%     if a==1
...
%     end
%     if counter>=1 && counter<=4 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
%         
%     end
%     if counter>=5 && counter<=8 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
%         
%     end
%     if counter>=9 && counter<=12  
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
%         
%     end
%     if counter>=13 && counter<=16 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
%         
%     end
%     if counter>=17 && counter<=20  
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
%         
%     end
%     if counter>=21 && counter<=24 
%         write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Blocked_RB_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
%     end
% end
