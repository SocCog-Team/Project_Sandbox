...
function []= rn_TrialWiseNEWPlotRecalibrated_poly_unblockedtrials_RB(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)

Byrewardttype_pos_unblocked_RB_Names=fieldnames(ModifiedTrialSets.Byrewardttype_pos.BySwitchingBlock.Unblocked.RB);
...
a=0;
for counter = 1:24
    Byrewardttype_posSp = Byrewardttype_pos_unblocked_RB_Names(counter);
    Byrewardttype_posSpStr = ModifiedTrialSets.Byrewardttype_pos.ByPostSwitch.Unblocked.RB.(Byrewardttype_posSp{1});
...
%   av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
	...
	OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.TargetOnset.xCoordinates(TrialLists,:),'.','Color','k','LineWidth',2);
	hold on
    OverLayPlotsTouchA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),epochdataTouchA.TargetOnset.xCoordinates(TrialLists,:),'.','Color','r','LineWidth',2);
    hold on
    OverLayPlotsTouchB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists,:),epochdataTouchB.TargetOnset.xCoordinates(TrialLists,:),'.','Color','b','LineWidth',2);
    hold on
	OverLayPlotsGazeAVGA=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AverageGazeA,'.','Color',[1,0.5,0],'LineWidth',2)
    hold on
    OverLayPlotsTouchAVGA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists(1),:),AverageTouchA,'.','Color','g','LineWidth',2)
    hold on
    OverLayPlotsTouchAVGB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists(1),:),AverageTouchB,'.','Color','y','LineWidth',2)
...
%   OverLayPlotsGazeA.Color='red';
    hold on 
	yyaxis right
	OverLayPlotsVergence = plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AverageVergence,'.','Color','m','LineWidth',2)
	set(gca,'ylim',[-50 50])
...
%     min(nnz(~isnan(epochdataTouchB.TargetOnset.timepoints(TrialLists,:))))
   ...
   hold on 
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightTop.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightCenter.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightBottom.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftTop.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftCenter.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftBottom.pdf'))
    end
end
...
% % % av=mean(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),1);
%     OverLayPlotsGazeA=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),epochdataRegisteredGazeA_poly_right.TargetOnset.yCoordinates(TrialLists,:),'.','Color','k','LineWidth',2);
%     hold on
% 	OverLayPlotsTouchA=plot(epochdataTouchA.TargetOnset.timepoints(TrialLists,:),epochdataTouchA.TargetOnset.xCoordinates(TrialLists,:),'.','Color','r','LineWidth',2);
%     hold on
%     OverLayPlotsTouchB=plot(epochdataTouchB.TargetOnset.timepoints(TrialLists,:),epochdataTouchB.TargetOnset.xCoordinates(TrialLists,:),'.','Color','b','LineWidth',2);
...
%     end
%     if counter>=1 && counter<=4 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightBottom.pdf'))
%         
%     end
%     if counter>=5 && counter<=8 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightCenter.pdf'))
%         
%     end
%     if counter>=9 && counter<=12  
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveRightTop.pdf'))
%         
%     end
%     if counter>=13 && counter<=16 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftBottom.pdf'))
%         
%     end
%     if counter>=17 && counter<=20  
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftCenter.pdf'))
%         
%     end
%     if counter>=21 && counter<=24 
%         write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Unblocked_RB_Red_ObjectiveLeftTop.pdf'))
%     end
end