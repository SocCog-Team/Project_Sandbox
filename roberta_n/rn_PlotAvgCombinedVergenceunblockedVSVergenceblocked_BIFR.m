function [] = rn_PlotAvgCombinedVergenceunblockedVSVergenceblocked_BIFR (FullStructure, FullStructure_unblocked,FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1:8
	...
	if a>=1 && a<=4
		annotation('textbox',[.4 .8 .9 .2],'String','Trials with red on objective right BIFR','EdgeColor','none')
		
	end
	if a>=5 && a<=8
		annotation('textbox',[.4 .8 .9 .2],'String','Trials with red on objective left BIFR','EdgeColor','none')
	end
	...
	if b==1
		ylabel('Avg Vergence unblocked and Vergence blocked BIFR','FontSize',9)
	...
	if a>=1 && a<=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergenceunblockedVSVergenceblocked_TimeRed_ObjectiveRightAlignedtoBIFR.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgVergenceunblockedVSVergenceblocked_TimeRed_ObjectiveLeftAlignedtoBIFR.pdf'))
	end
