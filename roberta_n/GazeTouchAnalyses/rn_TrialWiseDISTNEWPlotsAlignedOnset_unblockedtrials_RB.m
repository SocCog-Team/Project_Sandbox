...
function [DistGazeATouchA, DistGazeATouchB]= rn_TrialWiseDISTNEWPlotsAlignedOnset_unblockedtrials_RB(distGazeATouchA, distGazeATouchB,epochdataRegisteredGazeA_poly_right, ModifiedTrialSets, saving_dir, fileID)
close all
...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
    ...
    AbsDistGazeATouchAOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),distGazeATouchA(TrialLists,:),'.','Color','r');
    hold on
    AbsDistGazeATouchBOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),distGazeATouchB(TrialLists,:),'.','Color','b');
    hold on
    
	AbsDistGazeATouchAAVGOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),mean(distGazeATouchA(TrialLists,:)),'*','Color','r');
	hold on
	AbsDistGazeATouchBAVGOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),mean(distGazeATouchB(TrialLists,:)),'*','Color','b');
	hold on
...
%
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveRightTopAlignedtoOnset.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveRightCenterAlignedtoOnset.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveRightBottomAlignedtoOnset.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveLeftTopAlignedtoOnset.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBLocked_RB_Red_ObjectiveLeftCenterAlignedtoOnset.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_EuclDiffbwTouchTime_UnBlocked_RB_Red_ObjectiveLeftBottomAlignedtoOnset.jpg']))
    end
end
