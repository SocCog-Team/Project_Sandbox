function [] = rn_PlotAvgCombinedTouchBunblockedVSTouchBblocked (FullStructure, FullStructure_unblocked,FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1:8
	...
	B_blocked_isNotNaN_array = ~isnan(TrialLists_unblocked_B);
	...
	str_unblocked = strcat(TrialNumStr, num2str(size((TrialLists_unblocked_B),1)));
	str_blocked = strcat(TrialNumStr, num2str(size((TrialLists_blocked_B),1)));
	...
	hold on
	[AClosertoZero BClosertoZero] = tn_ClosertoZero(AvgB_unblocked,AvgB_blocked, TrialListsTimepoints);
	%      hold on
	...
	plot((TrialListsTimepoints(1,:)),AvgB_unblocked-SEM_B_unblocked,'Color','r', 'LineWidth',0.5);
	hold on
	plot((TrialListsTimepoints(1,:)),AvgB_blocked,'Color','b', 'LineWidth',2);
	plot((TrialListsTimepoints(1,:)),AvgB_blocked + SEM_B_blocked,'Color','b', 'LineWidth',0.5);
	plot((TrialListsTimepoints(1,:)),AvgB_blocked - SEM_B_blocked,'Color','b', 'LineWidth',0.5);
	hold on
	...
	%     plot((TrialListsTimepoints(6,:)),AvgAT_blocked - SEM_AT_blocked,'Color','m', 'LineWidth',0.5);
	%     hold on
	plot((TrialListsTimepoints(1,:)),AvgB_blocked + B_blocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	plot((TrialListsTimepoints(1,:)),AvgB_blocked - B_blocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	...
	if b==1
		ylabel('Avg Touch B unblocked and Touch B blocked','FontSize',9)
	...
	if a>=1 && a<=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchBunblockedVSTouchBblocked_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchBunblockedVSTouchBblocked_TimeRed_ObjectiveLeftAlignedtoTargetOnset.pdf'))
	end
...
ClosertoZeroY(ClosertoZeroY==0) = 650;
