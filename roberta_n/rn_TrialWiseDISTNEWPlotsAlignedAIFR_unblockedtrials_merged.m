...
for counter= 1:8
	...
	if counter>=1 && counter<=4
		annotation('textbox',[.2 .8 .9 .2],'String','Trials aligned to target onset in the unblocked condition : Red on Objective Right','EdgeColor','none')
	end
	if counter>=5 && counter<=8
		annotation('textbox',[.2 .8 .9 .2],'String','Trials aligned to target onset in the unblocked condition : Red on Objective Left','EdgeColor','none')
	end
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_unblocked_Red_ObjectiveRightAlignedtoAIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_unblocked_Red_ObjectiveLeftAlignedtoAIFR.pdf'))
