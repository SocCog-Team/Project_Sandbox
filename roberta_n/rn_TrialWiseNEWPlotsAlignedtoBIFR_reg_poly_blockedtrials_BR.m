...
%     OverLayPlotsTouchAVGB=plot(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists(1),:),mean(epochdataTouchB.B_InitialFixationReleaseTime_ms.xCoordinates(TrialLists,:),1),'*','Color','b' )
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightBottomAlignedtoBIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftBottomAlignedtoBIFR.jpg']))
    end
end
...
%     min(nnz(~isnan(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))))
    ...
    if counter>=1 && counter<=4 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightBottomAlignedtoBIFR.jpg']))
        
    end
    if counter>=5 && counter<=8 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=9 && counter<=12  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightTopAlignedtoBIFR.jpg']))
        
    end
    if counter>=13 && counter<=16 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftBottomAlignedtoBIFR.jpg']))
        
    end
    if counter>=17 && counter<=20  
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftCenterAlignedtoBIFR.jpg']))
        
    end
    if counter>=21 && counter<=24 
        saveas(gcf, fullfile(saving_dir,[fileID, '_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftTopAlignedtoBIFR.jpg']))
    end
end
