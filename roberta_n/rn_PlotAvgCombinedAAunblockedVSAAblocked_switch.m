function [] = rn_PlotAvgCombinedAAunblockedVSAAblocked_switch (FullStructure,FullStructure_unblocked_pre,FullStructure_unblocked_first,FullStructure_unblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1 : 8
   disp(['Processing condition: ', FullStructure_unblocked_first_AA{a}]);
	cur_fieldnames_unblocked_first_AA = FullStructure_unblocked_first_AA{a};
	
    
	if (a==3) ||(a==5)
		cur_fieldnames_unblocked_AA_pre = FullStructure_unblocked_pre_AA{a+1};
	else
		cur_fieldnames_unblocked_AA_pre = FullStructure_unblocked_pre_AA{a};
		
	end
	
    TrialLists_unblocked_AA = FullStructure_unblocked_pre.AA.(cur_fieldnames_unblocked_AA_pre);
	tmp_TrialLists_unblocked_AA = FullStructure_unblocked_first.AA.(cur_fieldnames_unblocked_first_AA);
	TrialListsTimepoints = FullStructure_unblocked.timepoints.TrialWise.(cur_fieldnames_unblocked_AA_pre);
	...
	tmp_AvgAA_unblocked = mean(tmp_TrialLists_unblocked_AA, 1, 'omitnan');
	tmp_STD_AA_unblocked = std(tmp_TrialLists_unblocked_AA, 'omitnan');
	tmp_SEM_AA_unblocked = std(tmp_TrialLists_unblocked_AA, 'omitnan') / size(tmp_TrialLists_unblocked_AA, 1);
	tmp_AA_unblocked_isNotNaN_array = ~isnan(tmp_TrialLists_unblocked_AA);
	tmp_AA_unblocked_N_per_bin_list = sum(tmp_AA_unblocked_isNotNaN_array, 1);
	[tmp_A_unblocked_CIHW95] = calc_cihw(tmp_STD_AA_unblocked, tmp_AA_unblocked_N_per_bin_list, 0.05);
	%   [A_CIHW95] = calc_cihw(STD_A, (size((TrialListsAA),1)), 0.05);
	...
	str=strcat(TrialNumStr, num2str(size((TrialLists_unblocked_AA),1)),TrialNumStr, num2str(size((tmp_TrialLists_unblocked_AA),1)));
	title (str);
	...
	hold on
	[AClosertoZero BClosertoZero] = tn_ClosertoZero(AvgAA_unblocked,tmp_AvgAA_unblocked, TrialListsTimepoints);
	%      hold on
	...
	plot((TrialListsTimepoints(1,:)),AvgAA_unblocked-SEM_AA_unblocked,'Color','r', 'LineWidth',0.5);
	hold on
	plot((TrialListsTimepoints(1,:)),tmp_AvgAA_unblocked,'Color','b', 'LineWidth',2);
	plot((TrialListsTimepoints(1,:)),tmp_AvgAA_unblocked + tmp_SEM_AA_unblocked,'Color','b', 'LineWidth',0.5);
	plot((TrialListsTimepoints(1,:)),tmp_AvgAA_unblocked - tmp_SEM_AA_unblocked,'Color','b', 'LineWidth',0.5);
	hold on
	...
	%     plot((TrialListsTimepoints(6,:)),AvgAT_blocked - SEM_AT_blocked,'Color','m', 'LineWidth',0.5);
	%     hold on
	plot((TrialListsTimepoints(1,:)),tmp_AvgAA_unblocked + tmp_A_unblocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	plot((TrialListsTimepoints(1,:)),tmp_AvgAA_unblocked - tmp_A_unblocked_CIHW95,'Color','b', 'LineWidth',0.5, 'LineStyle', '--');
	...
	if a>=1 && a<=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedSWITCHTimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAunblockedSWITCHTimeRed_ObjectiveLeftAlignedtoTargetOnset.pdf'))
	end
