...
function [DistGazeATouchA, DistGazeATouchB ,DistTouchBTouchA ]= rn_TrialWiseDISTNEWPlotsAlignedOnset(distGazeATouchA, distGazeATouchB,distTouchBTouchA,epochdataRegisteredGazeA_poly_right, ModifiedTrialSets, saving_dir)
...
%     line([AVGAIFR AVGAIFR],yL,'Color','r','LineStyle','--');
    ...
    DistGazeATouchAOverlay = plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),distGazeATouchA(TrialLists,:),'.','Color','r','MarkerSize',3);
    hold on
    DistGazeATouchBOverlay = plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),distGazeATouchB(TrialLists,:),'.','Color','b','MarkerSize',3);
    hold on
	DistTouchBTouchAOverlay = plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists,:),distTouchBTouchA(TrialLists,:),'.','Color','m', 'MarkerSize',3);
    hold on 
	DistGazeATouchAAVGOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AveragedistGazeATouchA,'*','Color',[1 0.5 0],'MarkerSize',3);
	hold on
	DistGazeATouchBAVGOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AveragedistGazeATouchB,'*','Color','y','MarkerSize',3);
	hold on
	DistTouchBTouchAAVGOverlay=plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AveragedistTouchBTouchA,'*','Color','g','MarkerSize',3);
	hold on
...
%
  ...
  if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTime_Red_ObjectiveRightTopAlignedtoOnset.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTime_Red_ObjectiveRightCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTime_Red_ObjectiveRightBottomAlignedtoOnset.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTime_Red_ObjectiveLeftTopAlignedtoOnset.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTimeRed_ObjectiveLeftCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'_EuclDiffbwTouchTime_Red_ObjectiveLeftBottomAlignedtoOnset.pdf'))
    end
end
