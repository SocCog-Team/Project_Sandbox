...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoAIFR_reg_poly(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)
...
for counter= 1:24
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveRightTopAlignedtoAIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveRightCenterAlignedtoAIFR.pdf'))
		
	end
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveRightBottomAlignedtoAIFR.pdf'))
	
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveLeftTopAlignedtoAIFR.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveLeftCenterAlignedtoAIFR.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Red_ObjectiveLeftBottomAlignedtoAIFR.pdf'))
	end
end
...
%     min(nnz(~isnan(epochdataTouchB.A_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))))
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveRightTopAlignedtoAIFR.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveRightCenterAlignedtoAIFR.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveRightBottomAlignedtoAIFR.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Red_ObjectiveLeftTopAlignedtoAIFR.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Red_ObjectiveLeftCenterAlignedtoAIFR.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_Red_ObjectiveLeftBottomAlignedtoAIFR.pdf'))
    end
end
