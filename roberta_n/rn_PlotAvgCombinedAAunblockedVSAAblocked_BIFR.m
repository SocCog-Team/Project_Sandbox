function [] = rn_PlotAvgCombinedAAunblockedVSAAblocked_BIFR (FullStructure, FullStructure_unblocked,FullStructure_blocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
for a = 1:8
	...
	if a==1
		annotation('textbox',[.2 .8 .9 .2],'String','Trials with red on Objective Right BIFR','EdgeColor','none')
		
	end
	if a==5
		annotation('textbox',[.2 .8 .9 .2],'String','Trials with red on Objective Left BIFR','EdgeColor','none')
	end
	...
	if a>=1 && a<=4
		 write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveRightAlignedtoBIFR.pdf'))
	end
	if a>=5 && a<=8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgDiffbwGazeA_unblockedTouchAblockedTimeRed_ObjectiveLeftAlignedtoBIFR.pdf'))
	end
