...
function [DistGazeATouchA, DistGazeATouchB,DistGazeA_finaltarget_A, DistTouchA_finaltarget_A]= rn_TrialWiseDISTNEWPlotsAlignedOnset_blockedtrials_BR(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
close all
...
for counter= 1:24
  ...
  if counter==1
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_BR_Red_ObjectiveRightTopAlignedtoOnset.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_BR_Red_ObjectiveRightCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_BR_Red_ObjectiveRightBottomAlignedtoOnset.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_BR_Red_ObjectiveLeftTopAlignedtoOnset.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_BLocked_BR_Red_ObjectiveLeftCenterAlignedtoOnset.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir,'EuclDiffbwTouchTime_Blocked_BR_Red_ObjectiveLeftBottomAlignedtoOnset.pdf'))
    end
end
