...
function [DistGazeATouchA, DistGazeATouchB,DistGazeA_finaltarget_A, DistTouchA_finaltarget_A]= tn_TrialWiseDISTNEWPlotsAlignedtoAIFR_unblockedtrials_BR(distGazeATouchA, distGazeATouchB,distGazeA_finaltarget_A, distTouchA_finaltarget_A,epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, ModifiedTrialSets, saving_dir)
close all
...
% 	DistTouchB_finaltarget_BAVGOverlay = plot (epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AveragedistTouchB_finaltarget_B,'*','Color',[1 0.5 0 ],'MarkerSize',6);
    ...
    if counter>=1 && counter<=4 
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveRightTopAlignedtoAIFR.pdf'))
        
    end
    if counter>=5 && counter<=8 
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveRightCenterAlignedtoAIFR.pdf'))
        
    end
    if counter>=9 && counter<=12  
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveRightBottomAlignedtoAIFR.pdf'))
        
    end
    if counter>=13 && counter<=16 
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveLeftTopAlignedtoAIFR.pdf'))
        
    end
    if counter>=17 && counter<=20  
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveLeftCenterAlignedtoAIFR.pdf'))
        
    end
    if counter>=21 && counter<=24 
        write_out_figure(gcf, fullfile(saving_dir, 'EuclDiffbwGazeTouchTime_Unblocked_BR_Red_ObjectiveLeftBottomAlignedtoAIFR.pdf'))
    end
end
