...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoAIFR_reg_poly_unblocked_BR_3D(epochdataRegisteredGazeA_poly_right, epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)

Byrewardttype_pos_unblocked_BR_Names=fieldnames(ModifiedTrialSets.Byrewardttype_pos.ByPostSwitch.Unblocked.BR);
...
for counter= 1:24
    ...
    Byrewardttype_posSpStr = ModifiedTrialSets.Byrewardttype_pos.ByPostSwitch.Unblocked.BR.(Byrewardttype_posSp{1});
...
%     end
    ...
    if counter>=1 && counter<=4
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveRightTop.pdf']))
        
    end
    if counter>=5 && counter<=8
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveRightCenter.pdf']))
        
    end
    if counter>=9 && counter<=12
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveRightBottom.pdf']))
        
    end
    if counter>=13 && counter<=16
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveLeftTop.pdf']))
        
    end
    if counter>=17 && counter<=20
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveLeftCenter.pdf']))
        
    end
    if counter>=21 && counter<=24
        write_out_figure(gcf, fullfile(saving_dir,[fileID, '_AIFR_3D_Unblocked_BR_Red_ObjectiveLeftBottom.pdf']))
    end
