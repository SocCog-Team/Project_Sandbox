function [] = rn_PlotAvgCombined_Touch_different_conditions_unblocked (FullStructure, FullStructure_unblocked, saving_dir)
b = 0;
TrialNumStr = 'N: ';
...
% 	end
	...
	if a >= 1 && a <=4
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_unblockedVSTouchAunblocked_mergedconditions_TimeRed_ObjectiveRightAlignedtoTargetOnset.pdf'))
	end

	
	if a >= 5 && a <= 8
		write_out_figure(gcf, fullfile(saving_dir,'_AvgTouchA_unblockedVSTouchAunblocked_mergedconditions_TimeRed_ObjectiveLefttAlignedtoTargetOnset.pdf'))
	end
