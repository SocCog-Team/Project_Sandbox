...
%%are later combined
function []= rn_TrialWiseNEWPlotsAlignedtoBIFR_reg_poly_unblockedtrials_RB(epochdataRegisteredGazeA_poly_right, epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir, fileID)
...
for counter= 1:24
	...
	Byrewardttype_posSpStr=ModifiedTrialSets.Byrewardttype_pos.BySwitchingBlock.Unblocked.RB.(Byrewardttype_posSp{1});
	...
	if counter>=1 && counter<=4
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
		
	end
	if counter>=5 && counter<=8
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
		
	end
	if counter>=9 && counter<=12
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
		
	end
	if counter>=13 && counter<=16
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
		
	end
	if counter>=17 && counter<=20
		write_out_figure(gcf, fullfile(saving_dir, '_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
		
	end
	if counter>=21 && counter<=24
		write_out_figure(gcf, fullfile(saving_dir,'_XRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
	end
end
...
% 	OverLayPlotsGazeAVGA=plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AverageGazeA,'.','Color',[1 0.5 0], 'MarkerSize',8)
% 	hold on
% 	OverLayPlotsTouchAVGA=plot(epochdataTouchA.timepoints(TrialLists(1),:),AverageTouchA,'*','Color','y', 'MarkerSize',8)
% 	hold on
% 	
% 	%
% 	%     MaxLengthTimePoints_idx=find(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists,:)==max(length(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))));
% 	%     min(nnz(~isnan(epochdataTouchB.B_InitialFixationReleaseTime_ms.timepoints(TrialLists,:))))
% 	OverLayPlotsTouchAVGB=plot(epochdataTouchB.timepoints(TrialLists(1),:),AverageTouchB,'*','Color','g', 'MarkerSize',8)
% 	hold on
% 	yyaxis right
% 	OverLayPlotsVergence = plot(epochdataRegisteredGazeA_poly_right.timepoints(TrialLists(1),:),AverageVergence,'.','Color','m', 'MarkerSize',8)
% 	set(gca,'ylim',[-50 50])
...
% 	end
% 	if counter>=1 && counter<=4
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightTopAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=5 && counter<=8
% 		write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightCenterAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=9 && counter<=12
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveRightBottomAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=13 && counter<=16
% 		write_out_figure(gcf, fullfile(saving_dir, '_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftTopAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=17 && counter<=20
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftCenterAlignedtoBIFR.pdf'))
% 		
% 	end
% 	if counter>=21 && counter<=24
% 		write_out_figure(gcf, fullfile(saving_dir,'_YRegPolyFixTime_UnBlocked_RB_Red_ObjectiveLeftBottomAlignedtoBIFR.pdf'))
% 	end
% end
% 
% % %%SAVE THE FIGURES !
