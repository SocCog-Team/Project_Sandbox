...
function []= rn_TrialWiseNEWPlotRecalibrated_poly_blockedtrials_BR(epochdataRegisteredGazeA_poly_right,epochdataRegisteredGazeA_poly_left, epochdataTouchA,epochdataTouchB, ModifiedTrialSets, saving_dir)
...
for counter = 1:24
	...
	if counter>=1 && counter<=4
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightTop.pdf'))
		
	end
	if counter>=5 && counter<=8
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightCenter.pdf'))
		
	end
	if counter>=9 && counter<=12
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveRightBottom.pdf'))
		
	end
	if counter>=13 && counter<=16
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftTop.pdf'))
		
	end
	if counter>=17 && counter<=20
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftCenter.pdf'))
		
	end
	if counter>=21 && counter<=24
		saveas(gcf, fullfile(saving_dir,'_XRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftBottom.pdf'))
	end
end
...
% 	hold on
% 	yyaxis right
% 	OverLayPlotsVergence = plot(epochdataRegisteredGazeA_poly_right.TargetOnset.timepoints(TrialLists(1),:),AverageVergence,'.','Color','m','MarkerSize',2)
% 	set(gca,'ylim',[-50 50])
% 	
% 	hold on
% 	if a==1
...
% 	end
% 	if counter>=1 && counter<=4
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightTop.pdf'))
% 		
% 	end
% 	if counter>=5 && counter<=8
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightCenter.pdf'))
% 		
% 	end
% 	if counter>=9 && counter<=12
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveRightBottom.pdf'))
% 		
% 	end
% 	if counter>=13 && counter<=16
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftTop.pdf'))
% 		
% 	end
% 	if counter>=17 && counter<=20
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftCenter.pdf'))
% 		
% 	end
% 	if counter>=21 && counter<=24
% 		saveas(gcf, fullfile(saving_dir,'_YRegPolyFixTime_Blocked_BR_Red_ObjectiveLeftBottom.pdf'))
% 	end
end